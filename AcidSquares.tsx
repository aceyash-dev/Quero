...
void main() {
  ...
  vec2 ndc =
    (2.0 * frag - iResolution.xy) / iResolution.y;
  ...
  float dent =
    exp(-dot(md, md) / (mr * mr)) *
    (3.0 * uMouseStrength * uEnableMouse * uMouseActive);
...
const AcidSquares: React.FC<AcidSquaresProps> = ({
  ...
  useEffect(() => {
    ...
    const pu =
      postProgram.uniforms as Record<
        string,
        { value: any }
      >;
    ...
    const ensureTargets = () => {
      if (!rtA) {
        ...
        rtA = new RenderTarget(gl, {
          width: bw,
          height: bh,
          depth: false,
        });

        rtB = new RenderTarget(gl, {
          width: bw,
          height: bh,
          depth: false,
        });
    ...
    const renderFrame = () => {
      ...
      if (blurRef.current > 0) {
        ...
        renderer.render({
          scene: mesh,
          target: rtA ?? undefined,
        });

        pu.uRadius.value =
          blurRef.current * 14.0;
    ...
    const setSize = () => {
      ...
      const pres =
        pu.iResolution.value as Float32Array;
    ...
    const loop = (t: number) => {
      ...
      (
        program.uniforms.uMouseActive as {
          value: number;
        }
      ).value =
        mouseActive.current;
      ...
      (
        program.uniforms.uEnableMouse as {
          value: number;
        }
      ).value =
        enableMouseRef.current
      ...
      (
        program.uniforms.uMouseStrength as {
          value: number;
        }
      ).value =
  ...
  return (
    <div
      ref={containerRef}
      className={`acid-squares-container ${className}`.trim()}
    />
  );
};
